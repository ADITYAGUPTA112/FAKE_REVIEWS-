"""
========================================================
  MODULE 3 â€” Reviewerâ€“Product Trust Graph
  Coordinated Review-Ring Detector using NetworkX
========================================================
Builds a bipartite graph (Users â†” Products) from review data,
then projects it to a userâ€“user co-review graph to surface
suspicious clusters ("review rings") where groups of users
have collectively reviewed the same set of obscure products â€”
a near-impossible coincidence in real traffic.

Pipeline:
    1. Build bipartite graph  Users â†” Products
    2. Identify "obscure" products (few unique reviewers)
    3. Project onto a Userâ€“User graph where edge weight =
       number of shared obscure products reviewed
    4. Filter to strong edges (â‰¥ min_shared_products)
    5. Detect connected components â†’ suspicious clusters
...
# â”€â”€ Tunable parameters â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
OBSCURE_MAX_REVIEWERS   = 50    # products reviewed by â‰¤ this many users â†’ "obscure"
...
def find_obscure_products(df: pd.DataFrame) -> set:
    """
    Returns product IDs reviewed by â‰¤ OBSCURE_MAX_REVIEWERS unique users.
...
def build_user_graph(
    ...
    """
    Projects bipartite data onto a Userâ€“User graph:
        - Each node = a user
        - Each edge = the two users co-reviewed â‰¥ 1 obscure product
        - Edge attribute ``shared_products`` lists all common obscure products
        - Edge attribute ``shared_count`` is the overlap size

    Only edges with shared_count â‰¥ MIN_SHARED_PRODUCTS are kept.
    """
...
def check_temporal_coordination(
    ...
    Returns a dict:
        coordinated_pairs : int   â€” # (user, product) pairs within the window
        coordination_rate : float â€” fraction of shared reviews that are timed
    """
    df = df.copy()
    df["timestamp"] = pd.to_datetime(df["timestamp"])
...
def detect_review_rings(
    ...
    # â”€â”€ Step 1: Identify obscure products â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    obscure = find_obscure_products(df)
    if verbose:
        print(f"  Step 1: {len(obscure)} obscure products "
              f"(â‰¤{OBSCURE_MAX_REVIEWERS} reviewers each)")

    if not obscure:
        print("No obscure products found; raise OBSCURE_MAX_REVIEWERS threshold.")
        return []

    # â”€â”€ Step 2: Build userâ€“user co-review graph â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    user_graph = build_user_graph(df, obscure)
    if verbose:
        print(f"  Step 2: Userâ€“user graph: "
              f"{user_graph.number_of_nodes()} nodes, "
              f"{user_graph.number_of_edges()} edges")

    # â”€â”€ Step 3: Find connected components â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    clusters = []
    ...
    # â”€â”€ Step 4: Sort by risk â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def print_ring_report(clusters: List[Dict]) -> None:
    ...
    if not clusters:
        print("âœ…  No review rings detected.")
        return

    print(f"\n{'='*60}")
    print(f"  REVIEW-RING DETECTOR â€” {len(clusters)} cluster(s) flagged")
    print(f"{'='*60}\n")

    for c in clusters:
        risk_icon = "ðŸ”´" if c["risk_score"] >= 75 else "ðŸŸ¡" if c["risk_score"] >= 45 else "ðŸŸ¢"
        ...
        print(f"     Product IDs         : {', '.join(c['shared_products'][:6])}"
              + ("â€¦" if len(c["shared_products"]) > 6 else ""))
        print()
...
if __name__ == "__main__":
    ...
    # â”€â”€ Ring A: 6 users all reviewing the same 4 obscure products â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Ring B: 5 users sharing 3 obscure products, loosely timed â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Normal users: spread across many popular products â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    print("=== Demo: Reviewerâ€“Product Trust Graph ===\n")
